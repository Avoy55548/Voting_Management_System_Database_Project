...
public class subquery2 extends JFrame{
	private Container c;
    ...
    subquery2(int x, int y)
	{
		...
		//************************** Table ************************************************
		String[] columnNames={"PARTY_NAME","PARTY_NOMINEE"};
	...
	private String[] [] getData1()
	{
		try {
            ...
            ResultSet rs = stmt.executeQuery("select party_name,party_nominee from ballot where party_nominee in (select candidate_name from candidates  where vote_count > 50)");
		 ...
		 String[][] data1 = new String[rowCount][2];
		  ...
		  rs = stmt.executeQuery("select party_name,party_nominee from ballot where party_nominee in (select candidate_name from candidates  where vote_count > 50)"); // Re-execute the query
