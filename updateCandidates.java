...
public class updateCandidates extends JFrame{
	private Container c;
	...
	updateCandidates(int x, int y)
	{
		...
		emailLabel.setBounds(40,250,300,50);
		...
		signpButton.addActionListener(new ActionListener(){
			...
			update(CANDIDATE_NAME, CANDIDATE_ID);
	 ...
	 private static void update(String CANDIDATE_NAME, int CANDIDATE_ID) {
        try {
            Class.forName("oracle.jdbc.driver.OracleDriver");
            Connection con = DriverManager.getConnection(
                    "jdbc:oracle:thin:@localhost:1521:xe", "system", "1234");
            PreparedStatement stmt = con.prepareStatement("update candidates set CANDIDATE_NAME = ? where CANDIDATE_ID = ?");
	...
	public static void main(String[] args)
		{
			new updateCandidates(1000,1000);
		}
