...
public class updateCenter extends JFrame{
	...
	updateCenter(int x, int y)
	{
	 ...
	 private static void update(String CENTER_NAME, int CENTER_ID) {
        try {
            Class.forName("oracle.jdbc.driver.OracleDriver");
            Connection con = DriverManager.getConnection(
                    "jdbc:oracle:thin:@localhost:1521:xe", "system", "1234");
            PreparedStatement stmt = con.prepareStatement("update center set CENTER_NAME = ? where CENTER_ID = ?");
            stmt.setString(1, CENTER_NAME);
            stmt.setInt(2, CENTER_ID);
            stmt.execute();
